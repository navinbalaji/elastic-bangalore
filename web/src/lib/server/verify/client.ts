...
function authHint(err: unknown): string {
...
function columnIndex(res: ESQLResult, name: string): number {
	return res.columns.findIndex((c) => c.name === name);
}

function hasTitleContaining(res: ESQLResult, substr: string): boolean {
...
function hasNonEmptyColumn(res: ESQLResult, name: string): boolean {
...
function hitContent(hit: Record<string, unknown>): string {
	const fields = hit.fields as Record<string, unknown> | undefined;
	if (!fields) return '';
	const content = fields.content as unknown[] | undefined;
	if (!content?.length) return '';
	return String(content[0] ?? '');
}

function topHitContent(result: Record<string, unknown>): string {
	const hits = result.hits as Record<string, unknown> | undefined;
	const hitList = hits?.hits as Record<string, unknown>[] | undefined;
...
export class VerifyClient {
	...
	async getFieldProperty(index: string, field: string): Promise<Record<string, unknown>> {
		const res = await this.es.indices.getMapping({ index });
		const idx = res[index];
		const props = idx?.mappings?.properties as Record<string, Record<string, unknown>> | undefined;
		const prop = props?.[field];
		if (!prop) throw new Error(`field ${field} not found in mapping`);
		return prop;
	}

	async getMappingFieldType(index: string, field: string): Promise<string> {
		const prop = await this.getFieldProperty(index, field);
		const typ = prop.type as string | undefined;
		if (!typ) throw new Error(`field ${field} has no type`);
		return typ;
	}

	async getSemanticTextInferenceID(index: string, field: string): Promise<string> {
		const prop = await this.getFieldProperty(index, field);
		const inf = prop.inference_id as string | undefined;
		if (!inf) throw new Error(`field ${field} has no inference_id`);
		return inf;
	}

	async rawSearch(index: string, body: Record<string, unknown>): Promise<Record<string, unknown>> {
		const res = await this.es.search({ index, body });
		return res as unknown as Record<string, unknown>;
	}

	async rawInference(inferenceId: string, input: string): Promise<Record<string, unknown>> {
	...
	async runESQL(query: string): Promise<ESQLResult> {
		...
		return res as unknown as ESQLResult;
	}

	private async indexExistsCheck(index: string): Promise<VerifyResult> {
		try {
	...
	private async docCountCheck(index: string, want: number): Promise<VerifyResult> {
		try {
	...
	private async docCountAtLeast(index: string, min: number): Promise<VerifyResult> {
		try {
			const count = await this.docCount(index);
			if (count < min) {
	...
	private async kibanaResource(path: string, label: string): Promise<VerifyResult> {
		try {
	...
	async verify(stepId: string): Promise<VerifyResult> {
		switch (stepId) {
			case 'm1-embeddings': {
				try {
					...
					const emb = out.text_embedding as unknown[] | undefined;
					if (!emb?.length) {
			...
			case 'm1-search': {
				try {
					...
					const hits = out.hits as Record<string, unknown> | undefined;
					const total = hits?.total as Record<string, unknown> | undefined;
					const value = (total?.value as number) ?? 0;
			...
			case 'm4-verify-workflow': {
				try {
					...
					const resp = JSON.parse(body) as {
						results?: { name?: string; definition?: { name?: string } }[];
					};
