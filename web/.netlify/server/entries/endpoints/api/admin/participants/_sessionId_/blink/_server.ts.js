...
const POST = async ({ cookies, params }) => {
  ...
  const rows = await db.select({ id: sessions.id, stuckAt: sessions.stuckAt }).from(sessions).where(eq(sessions.id, params.sessionId)).limit(1);
  const row = rows[0];
  if (!row) error(404, "Session not found");
  if (!row.stuckAt) return json({ error: "Participant is not stuck" }, { status: 400 });
  const now = /* @__PURE__ */ new Date();
  await db.update(sessions).set({ stuckAt: null, blinkAt: now, updatedAt: now }).where(eq(sessions.id, params.sessionId));
