...
function workshopComplete(states) {
...
function progressCounts(states) {
  let passed = 0;
  for (const st of states) {
...
function moduleProgress(states) {
  ...
  return MODULE_ORDER.map((module) => {
    ...
    let passed = 0;
    let hasFailed = false;
    const steps = moduleStates.map((st) => {
